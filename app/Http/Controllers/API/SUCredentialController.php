...
class SUCredentialController extends BaseController
{
    public function getSUCredentialList(Request $request)
    {

        $roleId = Auth::user()->user->role_id;
        $organizationId = Auth::user()->org_id;

        $sort = $request->has('sort') ? $request->get('sort') : 'credential.credential_id';
        ...
        $credentials = DB::table('lms_credentials as credential')
            ...
            ->select('credential.credential_id as credentialId', 'credential.credential_code as credentialCode', 'credential.credential_title as credentialTitle', 'credential.category_id as categoryName', 'credential.expiration_time as expirationTime', 'status.training_status as status')
            ->get();
        ...
        if ($credentials->count() > 0) {
            foreach ($credentials as $credential) {
                if (isset($credential->categoryName)) {
                    ...
                    $credential->categoryName = CategoryMaster::whereIn('category_id', $categoryId)->pluck('category_name');
                }
            }
        }
    ...
    public function addSUCredentials(Request $request)
    {
        ...
        try {
            ...
            //             $OrganizationCredentialCustomField->credential_id = $credential->id;
    ...
    public function getSUCredentialsById($credentialId)
    {
        try {
            $roleId = Auth::user()->user->role_id;
            $organizationId = Auth::user()->org_id;

            $credential = DB::table('lms_credentials as credential')
                //->leftjoin('lms_category_master as category','credential.category_id','=','category.category_id')
                ->leftjoin('lms_training_status as status', 'credential.status', '=', 'status.training_status_id')
                ->where(function ($query) use ($organizationId, $roleId) {
                    if ($roleId != 1) {
                ...
                ->select(
                    ...
                    'credential.credential_title as credentialTitle',
                    'credential.credential_code as credentialCode',
                    'credential.credential_note as credentialNote',
                    'credential.credential_description as credentialDescription',
                    'credential.expiration_time as expirationTime',
                    'credential.days_till_expiration as daysTillExpiration',
    ...
    public function updateSUCredentials(Request $request)
    {
        $authId = Auth::user()->user_id;
        $roleId = Auth::user()->user->role_id;
        $organizationId = Auth::user()->org_id;
        ...
        try {
            ...
            $credential = SUCredential::where('credential_id', $request->credentialId)
                ->where(function ($query) use ($organizationId, $roleId) {
                    if ($roleId != 1) {
                        $query->where('credential.org_id', $organizationId);
                    }
    ...
    public function deleteSUCredentialsById(Request $request)
    {
        try {
            $roleId = Auth::user()->user->role_id;
            $organizationId = Auth::user()->org_id;
    ...
    //             $credential = Credential::whereIn('id', $request->credentialIds)
    //                 ->where(function ($query) use ($organizationId, $roleId) {
    //                     if ($roleId != 1) {
    //                         $query->where('credential.org_id', $organizationId);
    //                     }
    ...
    //         $credential = Credential::where('id', $request->credentialId)
    //             ->where(function ($query) use ($organizationId, $roleId) {
    //                 if ($roleId != 1) {
    //                     $query->where('credential.org_id', $organizationId);
    //                 }
